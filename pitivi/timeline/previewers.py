...
class AudioPreviewer(Clutter.Actor, Zoomable):
    ...
    def startLevelsDiscovery(self, uri):
        self.peaks = None
        self.pipeline = Gst.parse_launch("uridecodebin uri=" + uri + " ! audioconvert ! level interval=10000000 post-messages=true ! fakesink name=fakelevelsink")

        sink = self.pipeline.get_by_name("fakelevelsink")
        sink.props.sync = True
        sink.set_qos_enabled(False)
